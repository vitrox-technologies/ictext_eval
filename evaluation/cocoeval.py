...
class COCOeval:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        if p.multi_label:
            det_aes_dic = {d['id']:d['aesthetic'] for d in dt}
        if not len(ious)==0:
        ...
        # calculate multi-label fscore for iou 0.5 only
        # consider maxdet = 100 and for all area
        if maxDet == 100 and aRng == [0 ** 2, 1e5 ** 2] and p.multi_label:
            # index 0 of gtm is for IOU 0.5
            for idx, det_id in enumerate(gtm[0]):
                # aesthetic labels for g
    ...
    def summarize(self):
        ...
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100 ):
            ...
            if ap == 2:
                ...
                types = [x + (' ' * (len(max(types, key=len)) - len(x))) for x in types]
                for idx, score in enumerate(m_scores):
                    print(iStr.format(titles[idx], types[idx], iouStr, areaRng, maxDets, score))
                return m_scores
            else:
        ...
        def _summarizeDets():
            ...
            if self.params.multi_label:
                m_scores = _summarize(2, iouThr=.5, maxDets=self.params.maxDets[2])
                stats = np.concatenate((stats, np.array(m_scores)))
            return stats
        def _summarizeKps():
            stats = np.zeros((10,))
